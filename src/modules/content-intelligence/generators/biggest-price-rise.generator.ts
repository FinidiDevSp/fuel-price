...
@Injectable()
export class BiggestPriceRiseGenerator implements InsightGenerator {
  readonly type = InsightType.BIGGEST_PRICE_RISE;
  ...
  async generate(_date: string): Promise<RawInsightData[]> {
    ...
    for (const fuelCode of KEY_FUELS) {
      ...
      const rises = movers.filter((m) => m.direction === 'up');
      if (rises.length === 0) continue;

      const biggest = rises[0];
